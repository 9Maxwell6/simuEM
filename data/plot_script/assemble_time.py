...
def main():
    ap = argparse.ArgumentParser(description="Plot assemble time vs h.")
    ...
    for path in args.files:
        h, t_asm = np.loadtxt(path, comments="#", usecols=(0, 5), unpack=True)
        order = np.argsort(h)
        h, t_asm = h[order], t_asm[order]
 
        label = os.path.basename(path).replace("_", r"\_")
 
        if args.no_fit:
            ax.loglog(h, t_asm, "+-", markersize=7, linewidth=0.7, label=label)
        else:
            if args.last_n is not None and args.last_n < len(h):
                h_fit, t_fit = h[:args.last_n], t_asm[:args.last_n]
            else:
                h_fit, t_fit = h, t_asm
            ...
            line, = ax.loglog(h, t_asm, "+-", markersize=7, linewidth=0.7,
                              label=f"{label}  ($\\sim h^{{{slope:.3f}}}$)")
            ax.loglog(h, fit, "--", linewidth=0.7, alpha=0.6, color=line.get_color())
 
            print(f"{label}: assemble time slope = {slope:.4f}")
 
    ax.set_xlabel(r"$h$")
    ax.set_ylabel(r"$t_{\text{assemble}}\ [\mathrm{s}]$")
