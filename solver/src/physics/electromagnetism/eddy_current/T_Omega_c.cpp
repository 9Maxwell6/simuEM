...
bool T_Omega_c::initialize_pc()
{
    ...
    bc_T_1_H1_v2_ = fe_system_.register_Dirichlet_BC(pc_LpM_, key_conductor_interface_1_, Dirichlet_Type::HOMOGENEOUS);
...
bool T_Omega_c::assemble_pc()
{
    ...
    PetscCall(MatPtAP(Mc_c_.mat,pc_P_.mat,MAT_INITIAL_MATRIX, PETSC_DEFAULT, &P21));
    PetscCall(MatPtAP(Kc_c_.mat,pc_P_.mat,MAT_INITIAL_MATRIX, PETSC_DEFAULT, &P22));

    la_kernel::create_nest_mat(2, 2, {P11, P12, P21, P22}, pc_PtP_.mat);

    la_kernel::destroy_mat(P11);
    la_kernel::destroy_mat(P12);
    la_kernel::destroy_mat(P21);
    la_kernel::destroy_mat(P22);
    ...
    if(pc_alg_==1){
        ...
        PetscCall(MatDuplicate(M1, MAT_COPY_VALUES,&pc_H1_.mat));
        PetscCall(MatAXPY(pc_H1_.mat, 1.0, M2, DIFFERENT_NONZERO_PATTERN));
        ...
        PetscCall(MatDuplicate(M1, MAT_COPY_VALUES,&pc_H2_.mat));
        PetscCall(MatAXPY(pc_H2_.mat, 1.0, M2, DIFFERENT_NONZERO_PATTERN));
...
{
    T_Omega_AMS_c::AMS_Info ams_info;
    ...
    if(pc_alg_ == 1){
        ...
        bc_T_1_H1_s_.apply_to_mat(pc_Qc_.mat);
        bc_O_o_c_.apply_to_mat(pc_Qi_.mat);
        bc_O_i_c_.apply_to_mat(pc_Qi_.mat);



        const Mat Mc  = br_system_.get_block_lhs(0,0);
        const Mat X_  = br_system_.get_block_lhs(0,1);
        const Mat Xt  = br_system_.get_block_lhs(1,0);
        const Mat Ki  = br_system_.get_block_lhs(1,1);
        const Mat Kc_r  = br_system_.get_block_lhs(0,2);
        const Mat Kc_c  = br_system_.get_block_lhs(2,0);

        petsc_util::petsc_save_ascii_mat(Mc, "mat_Mc.txt");
        petsc_util::petsc_save_ascii_mat(X_, "mat_X_.txt");
        petsc_util::petsc_save_ascii_mat(Xt, "mat_Xt.txt");
        petsc_util::petsc_save_ascii_mat(Ki, "mat_Ki.txt");
        petsc_util::petsc_save_ascii_mat(Kc_r, "mat_Kc.txt");
        petsc_util::petsc_save_ascii_mat(Kc_c, "mat_Kc_neg.txt");

        petsc_util::petsc_save_ascii_mat(pc_LpM_.mat, "pc_LpM.txt");
        petsc_util::petsc_save_ascii_mat(pc_Qc_.mat, "pc_Qc.txt");
        petsc_util::petsc_save_ascii_mat(pc_Qi_.mat, "pc_Qi.txt");
...
int main(int argc, char** argv) 
{
    ...
    if(h_refine)
    {
        const std::vector<std::pair<std::string, double>> mesh_sweep = {
            ...
            //{"test_bball_1.geo", 0.353500000},
            {"test_bball_2.geo", 0.250000000},
            {"test_bball_3.geo", 0.176776695},
            {"test_bball_4.geo", 0.125000000},
            {"test_bball_5.geo", 0.088388348},
            {"test_bball_6.geo", 0.062500000},
            {"test_bball_7.geo", 0.044200000},
            {"test_bball_8.geo", 0.031250000},
