...
T_Omega::T_Omega(Mesh& mesh, bool enable_preconditioner) : mesh_(mesh), enable_pc_(enable_preconditioner), fe_system_(mesh)
{
    ...
    H1_  = H1_Space(mesh.get_mesh_dimension(), 1);
    ...
    // for grad grad in coupling system
    Logger::info("[T_Omega preconditioner] - initialize Q_T.");
    pc_Q_T_ = fe_system_.register_dual_FE_space(H1_, H1_, key_conductor_1_, nullptr, nullptr);
    Logger::block_info(pc_Q_T_.id, 
                       pc_Q_T_.row_offset, 
                       pc_Q_T_.col_offset, 
                       pc_Q_T_.row_size, 
                       pc_Q_T_.col_size);

    Logger::info("[T_Omega preconditioner] - initialize Q_O.");
    pc_Q_O_ = fe_system_.register_dual_FE_space(H1_, H1_, key_Omega_, nullptr, nullptr);
    Logger::block_info(pc_Q_O_.id, 
                       pc_Q_O_.row_offset, 
                       pc_Q_O_.col_offset, 
                       pc_Q_O_.row_size, 
                       pc_Q_O_.col_size);

    Logger::info("[T_Omega preconditioner] - initialize G_T.");
    pc_G_T_ = fe_system_.register_FE_space_coupling(dof_T_1_, pc_Q_T_, key_conductor_1_);
    ...
    Logger::info("[T_Omega preconditioner] - initialize I_O.");
    pc_I_O_ = fe_system_.register_FE_space_coupling(dof_Omega_, pc_Q_O_, key_Omega_);
    Logger::block_info(pc_I_O_.id, 
                       pc_I_O_.row_offset, 
                       pc_I_O_.col_offset, 
                       pc_I_O_.row_size, 
                       pc_I_O_.col_size);
    ...
    Logger::info("[T_Omega preconditioner] - initialize J.");
    pc_J_ = fe_system_.register_FE_space_coupling(dof_Omega_, pc_L_, key_conductor_interface_layer_1_);
    Logger::block_info(pc_J_.id, 
                       pc_J_.row_offset, 
                       pc_J_.col_offset, 
                       pc_J_.row_size, 
                       pc_J_.col_size);

    Logger::info("[T_Omega preconditioner] - initialize J^T.");             
    pc_J_tp_ = fe_system_.transpose_block(pc_J_);
    Logger::block_info(pc_J_tp_.id, 
                       pc_J_tp_.row_offset, 
                       pc_J_tp_.col_offset, 
                       pc_J_tp_.row_size, 
                       pc_J_tp_.col_size);
    ...
    */
    ...
    //pc_br_system_ = fe_system_.initialize_block_rack(2, 2);
    
    //pc_br_system_.insert_block(pc_Q_O_,         0, 0);
    //pc_br_system_.insert_block(pc_L_,           1, 1);
    //pc_br_system_.insert_block(pc_J_,           0, 1);
    //pc_br_system_.insert_block(pc_J_tp_,        1, 0);
    //pc_br_system_.compute_block_offset();
    //Logger::info("[T_Omega] - initialize preconditioenr block rack: \n"+pc_br_system_.print_block_rack());
...
bool T_Omega::assemble_system()
{
    ...
    // 3D vector field.  -Hs = - ∇×∇×T - T + ∇Ω
    V_Field_function new_f_T(mesh_, [&](Eigen::Ref<const VectorXd> x, const Field_Data& fd, Eigen::Ref<VectorXd> v) {
        auto sx = std::sin(2*pi*x(0));
        auto sy = std::sin(2*pi*x(1));
        auto sz = std::sin(2*pi*x(2));
    ...
    // 3D vector field.  Hs = T - ∇Ω
    V_Field_function new_T_gradO(mesh_, [&](Eigen::Ref<const VectorXd> x, const Field_Data& fd, Eigen::Ref<VectorXd> v) {
        auto sx = std::sin(2*pi*x(0));
        auto sy = std::sin(2*pi*x(1));
        auto sz = std::sin(2*pi*x(2));
    ...
    // 3D vector field.  Hs = - ∇Ω
    V_Field_function new_gradO(mesh_, [&](Eigen::Ref<const VectorXd> x, const Field_Data& fd, Eigen::Ref<VectorXd> v) {
        auto sx = std::sin(2*pi*x(0));
        auto sy = std::sin(2*pi*x(1));
        auto sz = std::sin(2*pi*x(2));
    ...
    // 3D vector field.  Hs = ∇Ω
    V_Field_function new_neg_gradO(mesh_, [&](Eigen::Ref<const VectorXd> x, const Field_Data& fd, Eigen::Ref<VectorXd> v) {
        auto sx = std::sin(2*pi*x(0));
        auto sy = std::sin(2*pi*x(1));
        auto sz = std::sin(2*pi*x(2));
...
bool T_Omega::assemble_preconditioner()
{
    ...
    assemble_mat(fe_system_.assemble_mat_data(pc_Q_), [&](auto& e_data, auto& mat) {
        ...
        if(property_id == Domain::CONDUCTOR) Integrator__s_grad_S__grad_S::assemble_element_matrix(mu, e_data, mat);
    ...
    */
    ...
    Logger::info("[T_Omega - preconditioner] - assemble preconditioner Q_T.");
    assemble_mat(fe_system_.assemble_mat_data(pc_Q_T_), [&](auto& e_data, auto& mat) {
        double mu = 1;
        size_t property_id = e_data.e->get_property_id();
        if(property_id == Domain::CONDUCTOR) mu = 1.;
        else if(property_id == Domain::EMPTY) mu = 1.;
    ...
    Logger::info("[T_Omega - preconditioner] - assemble dof mapping I_O.");
    assemble_mat(fe_system_.assemble_mat_data(pc_I_O_), [&](auto& e_data, auto& mat) {
        Identity_Mapping::direct_mapping(e_data, mat);
    });
    ...
    Logger::info("[T_Omega - preconditioner] - assemble preconditioner J.");
    assemble_mat(fe_system_.assemble_mat_data(pc_J_), [&](auto& e_data, auto& mat) {
        double mu = 1;
        size_t property_id = e_data.e->get_property_id();
        Integrator_H1__s_V__grad_S::assemble_element_matrix(mu, e_data, mat);
        //Integrator_H1__s_V__grad_S::assemble_element_matrix(-mu, e_data, mat);
    });

    pc_J_tp_.block_transpose(pc_J_);


    Logger::info("[T_Omega - preconditioner] - build preconditioner linear system.");
    pc_br_system_.build_linear_system();

    petsc_util::petsc_save_ascii_mat(pc_br_system_.get_lhs(), "PC_system.txt");

    Logger::info("[T_Omega - preconditioner] - apply Dirichlet BC to preconditioner linear system.");

    pc_bc_Oo_H1_.apply_to_mat(pc_br_system_.get_lhs());
    pc_bc_Oi_H1_.apply_to_mat(pc_br_system_.get_lhs());
    pc_bc_T_1_v_.apply_to_mat(pc_br_system_.get_lhs());
